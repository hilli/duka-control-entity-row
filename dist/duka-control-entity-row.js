    ...
    return Polymer.html`
			...
			<hui-generic-entity-row hass="[[hass]]" config="[[_config]]">
				<div class='horizontal justified layout' on-click="stopPropagation">
					<button class='button' style='[[_modeInColor]]' toggles name="in" on-click='setMode'>
					...
					<button class='button' style='[[_modeInOutColor]]' toggles name="inout" on-click='setMode'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
					...
					<button class='button' style='[[_modeOutColor]]' toggles name="out" on-click='setMode'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
...
<!--
					<button class='button' style='[[_offColor]]' name="off" on-click='setSpeed' disabled='[[_isOffState]]'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
...
-->
					<button class='button' style='[[_lowOnColor]]' name="low" on-click='setSpeed' disabled='[[_isOnLow]]'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
					...
					<button class='button' style='[[_medOnColor]]' name="medium" on-click='setSpeed' disabled='[[_isOnMed]]'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
					...
					<button class='button' style='[[_highOnColor]]' name="high" on-click='setSpeed' disabled='[[_isOnHigh]]'>
						<svg style="width:24px;height:24px" viewBox="0 0 24 24">
    ...
    if (stateObj && stateObj.attributes) {
      ...
      if (config.customTheme) {
        modeincolor =
          'background-color:' +
          (mode == 'in'
            ? config.customIsModeOnColor
            : config.customIsOffSpdColor)
        modeinoutcolor =
          'background-color:' +
          (mode == 'inout'
            ? config.customIsModeOnColor
        ...
        lowcolor =
          'background-color:' +
          (speed == 'low'
            ? config.customIsOnLowColor
            : config.customIsOffSpdColor)
        medcolor =
          'background-color:' +
          (speed == 'medium'
            ? config.customIsOnMedColor
            : config.customIsOffSpdColor)
        hicolor =
          'background-color:' +
          (speed == 'high'
            ? config.customIsOnHiColor
            : config.customIsOffSpdColor)
        offcolor =
          'background-color:' +
          (speed == 'off'
            ? config.customIsOffColor
            : config.customIsOffSpdColor)
      } else {
        modeincolor =
          'background-color:' +
          (mode == 'in' ? 'var(--primary-color)' : 'var(--disabled-text-color)')
        modeinoutcolor =
          'background-color:' +
          (mode == 'inout'
            ? 'var(--primary-color)'
  ...
  setMode (e) {
    ...
    this.hass.callService('dukaone', 'set_mode', {
      entity_id: this._config.entity,
      mode: mode
    })
  }
